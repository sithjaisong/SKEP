        ...
        if (length(ls(envir = globalenv())) > 0) {
                packageStartupMessage(
                        make_pretty("Hi! I see that you have some variables saved in your",
                                    ...
                                    "before starting to query the SKEP data.", skip_after = TRUE),
                ...
                packageStartupMessage(
                        make_pretty("Wellcome to SKEP database! Please, type ?loginSKEP() to see how you can access the database.",
                                    skip_after = TRUE)
                )
        }
        invisible()
