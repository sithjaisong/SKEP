        ...
        if (length(ls(envir = globalenv())) > 0) {
                packageStartupMessage(
                        make_pretty("Hi! I see that you have some variables saved in your",
                                    ...
                                    "before starting to query the SYT SKEP data.", skip_after = TRUE),
                        ...
                        make_pretty("Type ?loginSKEP() to see how you access to the database.", skip_after = TRUE)
                )
        } else {
                packageStartupMessage(
                        make_pretty("Welcome to the SYT SKEP database! Please, type ?loginSKEP() to see how you can access the database.",
                                    skip_after = TRUE)
                )
        }
        invisible()
