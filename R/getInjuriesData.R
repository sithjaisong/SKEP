...
getInjuriesData <- function(x) {
        ...
        injury_info <- tbl(x, "crop_injuries") %>% collect() %>% select(-c(project, 
                                                                           field_num))
        
        hill.quad <- tbl(x, "hill_quad") %>% collect() %>% select(-id_hq) %>% 
        ...
        # reference of tyope of hill data type_hq <- tbl(mydb, 'type_hq')
        ...
        injuries <- tbl(x, "injury") %>% collect() %>% select(-id_pest_tp) %>% 
